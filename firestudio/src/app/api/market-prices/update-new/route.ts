...
// Comprehensive production-ready implementation with API + scraping fallbacks
// ============================================================================

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import * as cheerio from 'cheerio';
import type { UpdateMarketPricesResponse } from '@/types/market-prices';
...
async function scrapeMarketData(target: any): Promise<any[]> {
  try {
    ...
    $('table tr, .price-row, .market-item').each((index, element) => {
      try {
...
async function upsertToSupabase(data: any[]): Promise<{ success: number; failed: number; errors: string[] }> {
  ...
  for (let i = 0; i < data.length; i += batchSize) {
    ...
    try {
      ...
      results.failed += batch.length;
      results.errors.push(`Batch ${i}-${i + batch.length}: ${error.message || String(error)}`);
    }
  }
...
export async function GET() {
  return NextResponse.json({
    message: 'Market Prices Update API - Use POST to trigger updates',
    endpoints: {
      'POST /api/market-prices/update': 'Update market prices from all sources',
      'GET /api/market-prices': 'Get market prices with filters'
    },
    sources: API_SOURCES.map(s => ({ name: s.name, active: s.active })),
    scrapingTargets: SCRAPING_TARGETS.map(t => ({ state: t.state, url: t.url })),
