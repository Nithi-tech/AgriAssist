  ...
  const handleLanguageChange = async (languageCode) => {
    try {
      ...
      toast({
        ...
        description: `${t('language_changed_to') || 'Language changed to'} ${selectedLanguage?.native}`,
      });
    ...
    <Card>
      <CardContent className="p-6">
        ...
        <div className="mt-4 text-xs text-muted-foreground">
          {isOpen 
            ? t('click_language_select') || 'Click on a language to select it' 
            : `${t('currently_using') || 'Currently using'}: ${currentLang.native} (${currentLang.name})`
          }
        </div>
      </CardContent>
    </Card>
